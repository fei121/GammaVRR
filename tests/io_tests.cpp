...
class TemporaryDirectory {
  public:
    TemporaryDirectory() {
        ...
        path_ =
            std::filesystem::temp_directory_path() / ("refreshtone-tests-" + std::to_string(suffix));
...
TEST_CASE("P3 comments are accepted and images round-trip") {
    ...
    const auto image = refreshtone::read_ppm(input_path);
    REQUIRE(image.width == 2);
    REQUIRE(image.height == 1);
    REQUIRE(image.max_value == 4095);
    REQUIRE(image.format == refreshtone::PpmFormat::p3);
    REQUIRE(image.samples == std::vector<std::uint16_t>{0, 1, 2, 4093, 4094, 4095});

    refreshtone::write_ppm(image, output_path);
    const auto round_trip = refreshtone::read_ppm(output_path);
...
TEST_CASE("12-bit P6 samples use portable big-endian encoding") {
    ...
    const refreshtone::Image image{
        1, 1, 4095, refreshtone::PpmFormat::p6, {0x000, 0x123, 0xFFF},
    };

    refreshtone::write_ppm(image, path);
    const auto round_trip = refreshtone::read_ppm(path);
    REQUIRE(round_trip.format == refreshtone::PpmFormat::p6);
    REQUIRE(round_trip.samples == image.samples);
}
...
TEST_CASE("three signed LUT channels are loaded in level-major order") {
    ...
    const auto lut = refreshtone::read_lut_files(red, green, blue);
    REQUIRE(lut[0][0][0] == 1);
...
TEST_CASE("invalid LUT sizes and out-of-range PPM samples are rejected") {
    ...
    REQUIRE_THROWS(refreshtone::read_lut_files(short_lut, short_lut, short_lut));
    REQUIRE_THROWS(refreshtone::read_ppm(image_path));
}
