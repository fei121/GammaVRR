...
[[nodiscard]] std::uint16_t reference_sample(const gammavrr::Lut &lut, std::uint16_t sample,
    ...
    return static_cast<std::uint16_t>(
        std::clamp(static_cast<std::int32_t>(sample) + offset, 0,
                   static_cast<std::int32_t>(gammavrr::kMaxSampleValue)));
}

[[nodiscard]] gammavrr::Lut varied_lut() {
    gammavrr::Lut lut{};
...
TEST_CASE("constant offsets are applied per RGB channel") {
    ...
    const gammavrr::Model model({true, 5}, lut);
    ...
    REQUIRE(error == gammavrr::ProcessError::none);
...
TEST_CASE("two-stage interpolation matches the fixed golden vector") {
    ...
    const gammavrr::Model model({true, 3}, lut);
    const std::vector<std::uint16_t> input{24, 24, 4095};
    std::vector<std::uint16_t> output(3);

    REQUIRE(model.process({input, 1, 1}, 24, {output, 1, 1}) == gammavrr::ProcessError::none);
...
TEST_CASE("interpolation rounds exact halves away from zero") {
    ...
    const gammavrr::Model model({}, lut);
    const std::vector<std::uint16_t> input{8, 8, 8};
    std::vector<std::uint16_t> output(3);

    REQUIRE(model.process({input, 1, 1}, 0, {output, 1, 1}) == gammavrr::ProcessError::none);
    REQUIRE(output[0] == 9);
    REQUIRE(output[1] == 7);
    REQUIRE(output[2] == 8);
}
...
TEST_CASE("disabled model validates then copies input and supports in-place processing") {
    const gammavrr::Model model({false, 123}, varied_lut());
    std::vector<std::uint16_t> samples{0, 16, 4095, 100, 200, 300};
    const auto original = samples;

    REQUIRE(model.process({samples, 2, 1}, 127, {samples, 2, 1}) == gammavrr::ProcessError::none);
    REQUIRE(samples == original);
}

TEST_CASE("validation errors do not modify output") {
    const gammavrr::Model model({}, varied_lut());
...
TEST_CASE("bit-exact model matches an independent floating-point oracle exhaustively") {
    ...
    const gammavrr::Model model({true, zero_setting}, lut);

    std::vector<std::uint16_t> input(gammavrr::kNodeCount * 16 * gammavrr::kChannelCount);
    std::vector<std::uint16_t> output(input.size());
    for (std::size_t sample = 0; sample <= gammavrr::kMaxSampleValue; ++sample) {
        for (std::size_t channel = 0; channel < gammavrr::kChannelCount; ++channel) {
            input[sample * gammavrr::kChannelCount + channel] = static_cast<std::uint16_t>(sample);
        }
    }

    for (int frame_level = gammavrr::kMinFrameLevel; frame_level <= gammavrr::kMaxFrameLevel;
         ++frame_level) {
        REQUIRE(model.process({input, gammavrr::kNodeCount * 16, 1}, frame_level,
                              {output, gammavrr::kNodeCount * 16, 1}) ==
                gammavrr::ProcessError::none);

        for (std::size_t sample = 0; sample <= gammavrr::kMaxSampleValue; ++sample) {
            for (std::size_t channel = 0; channel < gammavrr::kChannelCount; ++channel) {
                REQUIRE(output[sample * gammavrr::kChannelCount + channel] ==
